...
public class open_Application_InterfaceForm extends org.apache.struts.action.ActionForm {

    private FormFile photo, imgqul, imgcaste, memo,castecertificate,exservicecertificate,physicallycertificate;
    FormFile ssccertificate;
    private String iti_code, iti_name, exs, ossc, type,ssc_type,email,email_id,adar,mobile;
    ...
    String name, ssc, tot_marks, tot_gpa, tot_grade,phno;
    ...
    public ActionErrors validate(ActionMapping mapping, HttpServletRequest request){
        System.out.println("U R IN open_Application_sadffsfsfsd");
        
        HttpSession session = request.getSession(true);
        
        ActionErrors errors = new ActionErrors();
        
        if(getAction_type().equalsIgnoreCase("app_submit")){
            ...
            //ssc_month
            System.out.println("sdffsdfsdfsrterte"+getSsc_month());
            if(getSsc_month() == null || getSsc_month().length()<1){
                errors.add("ssc_month", new ActionMessage("error.ssc_month.required"));
            } else if(getSsc_month().equals("03") || getSsc_month().equals("04")){ }
            else { errors.add("ssc_month", new ActionMessage("error.ssc_month.notmatch")); }
            
            if(getPhoto() == null){
                errors.add("photo", new ActionMessage("error.photo.required"));
            }
            ...
            if(getName() == null || getName().length()<1){
                ...
                errors.add("name", new ActionMessage("error.name.notmatched"));
            }
            
            //phno
            if(getPhno() == null || getPhno().length()<1){
                errors.add("phno", new ActionMessage("error.phno.required"));
            }else if(!Validations.Validate(getPhno(), 10)){
                errors.add("phno", new ActionMessage("error.phno.notmatched"));
            }
            ...
            if(getDob() == null || getDob().length()<1){
                ...
                errors.add("dob", new ActionMessage("error.dob.notmatched"));
            }
        
            //adarno
            if(getAdarno() == null || getAdarno().length()<1){
                errors.add("adarno", new ActionMessage("error.adarno.required"));
            }else if(!Validations.Validate( getAdarno(), 32)){
                errors.add("adarno", new ActionMessage("error.adarno.notmatched"));   
            }
        
            //gen
            if(getGen() == null || getGen().length()<1){
                errors.add("gen", new ActionMessage("error.gen.required"));
            }else if(getGen().equals("male") || getGen().equals("female")){ }
            else { errors.add("gen", new ActionMessage("error.gen.notmatched")); }
        
            //caste
            if(getCaste() == null || getCaste().length()<1){
                errors.add("caste", new ActionMessage("error.caste.error"));
            }else if(!Validations.Validate(getCaste(), 14)){
                errors.add("caste", new ActionMessage("error.caste.matched"));
            }
            ...
            if(getFname() == null || getFname().length()<1){
                ...
                errors.add("fname", new ActionMessage("error.fname.notmatched"));
            }
            
            //phc
            if(getPhc() == null || getPhc().length()<1){
                errors.add("phc", new ActionMessage("error.phc.required"));
            }else if(getPhc().equals("yes") || getPhc().equals("no")){ }
            else { errors.add("phc", new ActionMessage("error.phc.notmatched")); }
        
            //mname
            if(getMname() == null || getMname().length()<1){
                errors.add("mname", new ActionMessage("error.mname.required"));
            }else if(!Validations.Validate(getMname(), 3)){
                errors.add("mname", new ActionMessage("error.mname.notmatched"));
            }
        
            //getPwd_category()
            if(getPhc().equals("yes")){
                if(getPwd_category() == null || getPwd_category().length()<1){
                    errors.add("pwd_category", new ActionMessage("error.pwd_category.required"));
                }
                else if(!Validations.Validate(getPwd_category(), 3)){
                    errors.add("pwd_category", new ActionMessage("error.pwd_category.notmatched"));
                }
            }
            
            //exs
            if(getExs() == null || getExs().length()<1){
                errors.add("exs", new ActionMessage("error.exs.required"));
            }else if(getExs().equals("yes") || getExs().equals("no")){ }
            else { errors.add("exs", new ActionMessage("error.exs.notmatched")); }
            
            //ssc
            if(getSsc() == null || getSsc().length()<1){
                errors.add("ssc", new ActionMessage("error.ssc.required"));
            }else if(getSsc().equals("yes") || getSsc().equals("no")){ }
            else { errors.add("ssc", new ActionMessage("error.ssc.notmatched")); }
        
            //addr
            if(getAddr() == null || getAddr().length()<1){
                errors.add("addr", new ActionMessage("error.addr.required"));
            }else if(!Validations.Validate(getAddr(), 12)){
                errors.add("addr", new ActionMessage("error.addr.notmatched"));
            }
        
            //getLoconoc()
            if(getLoconoc() == null || getLoconoc().length()<1){
                errors.add("loconoc", new ActionMessage("error.loconoc.required"));
            }else if(getLoconoc().equals("Local") || getLoconoc().equals("NonLocal")){ }
            else { errors.add("loconoc", new ActionMessage("error.loconoc.notmatched")); }
            
            //pincode
            if(getPincode() == null || getPincode().length()<1){
                errors.add("pincode", new ActionMessage("error.pincode.required"));
            }else if(!Validations.Validate(getPincode(), 11)){
                errors.add("pincode", new ActionMessage("error.pincode.notmatched"));
            }
        
            //email
            if(getEmail() == null || getEmail().length()<1){
                errors.add("email", new ActionMessage("error.email.required"));
            }else if(!Validations.Validate(getEmail(), 33)){
                errors.add("email", new ActionMessage("error.email.notmatched"));
            }
        
            //ews
            if(getEws() == null || getEws().length()<1){
                errors.add("ews", new ActionMessage("error.ews.required"));
            }else if(getEws().equals("yes") || getEws().equals("no")){ }
            else { errors.add("ews", new ActionMessage("error.ews.notmatched")); }
        
            //subjects
            if(getSubjects() == null || getSubjects().length()<1){
                errors.add("subjects", new ActionMessage("error.subjects.required"));
            }
        }
