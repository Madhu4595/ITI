...
public class UpdateForm extends org.apache.struts.action.ActionForm {
    ...
    public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
        ...
        try {
            ...
            if (role_id.equals("3")) {
                nm=beans.MyUtil.getOptionValue("application", "regid", "name", regid, con);
//                nm = beans.MyUtil.getOptionValue("application", "regid", "name", regid, "dist_code", iti_code, con);
            } else if (role_id.equals("4")) {
                nm = beans.MyUtil.getOptionValue("application", "regid", "name", regid, "iti_code", iti_code, con);
...
System.out.println("dlfkjgvlfdjgdf"+nm);
            if (!(nm.length() > 1)) {
