/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package gov.ntaa.info;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionForward;
import org.apache.struts.action.ActionMapping;

/**
 *
 * @author theru hareesh
 */
public class GenerateAction extends org.apache.struts.action.Action {

    /* forward name="success" path="" */
    // private static final String SUCCESS = "success";
    /**
     * This is the action called from the Struts framework.
     *
     * @param mapping The ActionMapping used to select this instance.
     * @param form The optional ActionForm bean for this request.
     * @param request The HTTP Request we are processing.
     * @param response The HTTP Response we are processing.
     * @throws java.lang.Exception
     * @return
     */
    @Override
    public ActionForward execute(ActionMapping mapping, ActionForm form,
            HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        HttpSession session = request.getSession(true);
        NewTradeFormBean ntf = (NewTradeFormBean) form;
        //int trade_code = ntf.getTrade_code();
        String trade_short = ntf.getTrade_short();
        /*String trade_name = ntf.getTrade_name();
         int duration_yrs = ntf.getDuration_yrs();
         String eng_noneng = ntf.getEng_noneng();
         String min_qual = ntf.getMin_qual();*/

        NewTradeDTO ntd = new NewTradeDTO();

        ntd.setTrade_short(trade_short);

        System.out.println("the value of trade short:" + ntd.getTrade_short());
        NewTradeDAO nd = new NewTradeDAO();
        int trade_code = nd.genTradeCode(ntd,request);
        session.setAttribute("trade", trade_code);
        return mapping.findForward("abc");


        /*int trade_code=nd.retrieve(ntd);
         session.setAttribute("new", trade_code);
         return mapping.findForward("abc");*/
    }
}
